...
use smithay::{
    ...
    utils::{Logical, Point, Rectangle, Scale, Serial, Size},
};

use crate::{
    animation::{Animation, Clock},
    niri_render_elements,
    render_helpers::{renderer::NiriRenderer, RenderTarget},
    utils::{
        center_preferring_top_left_in_area, clamp_preferring_top_left_in_area, ensure_min_max_size,
        ensure_min_max_size_maybe_zero, transaction::TransactionBlocker, ResizeEdge,
    },
    window::ResolvedWindowRules,
};

use super::{
    closing_window::{ClosingWindow, ClosingWindowRenderElement},
    floating::DIRECTIONAL_MOVE_PX,
    scrolling::ColumnWidth,
    tile::{Tile, TileRenderElement, TileRenderSnapshot},
    workspace::{InteractiveResize, ResolvedSize},
    ConfigureIntent, InteractiveResizeData, LayoutElement, Options, RemovedTile, SizeFrac,
};

/// Space for pinned windows.
#[derive(Debug)]
pub struct PinnedSpace<W: LayoutElement> {
...
impl<W: LayoutElement> PinnedSpace<W> {
    pub fn new(
        view_size: Size<f64, Logical>,
        working_area: Rectangle<f64, Logical>,
        scale: f64,
    ...
    pub fn render_elements<R: NiriRenderer>(
        ...
        target: RenderTarget,
        focus_ring: bool,
    ) -> Vec<PinnedSpaceRenderElement<R>> {
        let mut rv = Vec::new();
    ...
    pub fn get_active_center(&self) -> Option<Point<f64, Logical>> {
        let Some(active_id) = &self.active_window_id else {
            return None;
        };
        let active_idx = self.idx_of(active_id).unwrap();
